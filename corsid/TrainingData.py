...
class PairwiseTrainingData:
    """
    Structure for the training data used for system identification. Here, the inputs are
    assumed to be applied in a pairwise fashion:
    """
    def __init__(
            self,
            num_pix: int = None,    # Number of pixels in dark zone
            ...
            num_iter: int = None,   # Number of iterations of training data (time steps)
            zs: dict = None,        # Data vectors for each iteration
            bs: dict = None,        # Control vectors for each PAIR of iterations
            Psi: np.ndarray = None  # Probe command matrix (len_u, len_z). Each probe is a column.
    ):
        self.num_pix = num_pix
        self.len_x = len_x
